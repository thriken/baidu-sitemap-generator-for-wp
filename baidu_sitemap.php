...
Description: This pulgin generates a Baidu XML-Sitemap for WordPress Blog. Also Build a real Static Sitemap-Page for all Search Engine. | 生成百度 Sitemap XML 文件。就相当于网站被百度--全球最大的中文搜索引擎订阅，进而为您的网站带来潜在的流量。同时生成一个静态的站点地图页面，对所有的搜索引擎都有利。现在支持php7.4-php8.0
Author: ELEC.TOP
Version: 1.51
Fix Author URI: https://blog.elec.top
*/
...
function get_baidu_sitemap_options(){
	...
	if( $get_baidu_sitemap_options ){
		...
		$array_baidu_sitemap_options['lc_is_Enabled_Html_Sitemap'] = '0';
		...
		$array_baidu_sitemap_options['lc_tag_select'] = '0';
		$array_baidu_sitemap_options['is_sina_sae'] = '0';
	}
...
function update_baidu_sitemap_options() {
	if ($_POST['action']=='build_options'){
		$lc_is_Enabled_XML_Sitemap = isset($_POST['lc_is_Enabled_XML_Sitemap']) ? $_POST['lc_is_Enabled_XML_Sitemap'] : 0;
		if(!$lc_is_Enabled_XML_Sitemap){ $lc_is_Enabled_XML_Sitemap = 0; }
		$lc_is_Enabled_Html_Sitemap = isset($_POST['lc_is_Enabled_Html_Sitemap']) ? $_POST['lc_is_Enabled_Html_Sitemap'] : 0;
		if(!$lc_is_Enabled_Html_Sitemap){ $lc_is_Enabled_Html_Sitemap = 0; }
		$lc_is_update_sitemap_when_post = isset($_POST['lc_is_update_sitemap_when_post']) ? $_POST['lc_is_update_sitemap_when_post'] : 0;
		if(!$lc_is_update_sitemap_when_post){ $lc_is_update_sitemap_when_post = 0; }
		$lc_post_limit1000 = isset($_POST['lc_post_limit1000']) ? $_POST['lc_post_limit1000'] : 0;
		if(!$lc_post_limit1000){ $lc_post_limit1000 = 0; }
		################## NEW~
		$lc_post_select = isset($_POST['lc_post_select']) ? $_POST['lc_post_select'] : 1;
		if(!$lc_post_select){ $lc_post_select = 1; }
		$lc_page_select = isset($_POST['lc_page_select']) ? $_POST['lc_page_select'] : 0;
		if(!$lc_page_select){ $lc_page_select = 0; }
		$lc_category_select = isset($_POST['lc_category_select']) ? $_POST['lc_category_select'] : 0;
		if(!$lc_category_select){ $lc_category_select = 0; }
		$lc_tag_select = isset($_POST['lc_tag_select']) ? $_POST['lc_tag_select'] : 0;
		if(!$lc_tag_select){ $lc_tag_select = 0; }
		##################
		$is_sina_sae = isset($_POST['is_sina_sae']) ? $_POST['is_sina_sae'] : 0;
